...
writemeteorologygridpixel<-function(file, index, data, verbose = FALSE) {
  ...
  i = which(cv[[1]]==cci[1])
  j = which(cv[[2]]==cci[2])
  if("MeanTemperature" %in% names(data)) .putvardatapixel(nc,ny, nt, "MeanTemperature", i, j, data$MeanTemperature)
  if("MinTemperature" %in% names(data)) .putvardatapixel(nc,ny, nt, "MinTemperature", i, j, data$MinTemperature)
  if("MaxTemperature" %in% names(data)) .putvardatapixel( nc, ny, nt, "MaxTemperature", i, j, data$MaxTemperature)
  if("Precipitation" %in% names(data)) .putvardatapixel( nc, ny, nt, "Precipitation", i, j, data$Precipitation)
  if("MeanRelativeHumidity" %in% names(data)) .putvardatapixel( nc, ny, nt, "MeanRelativeHumidity", i, j, data$MeanRelativeHumidity)
  if("MinRelativeHumidity" %in% names(data)) .putvardatapixel( nc, ny, nt, "MinRelativeHumidity", i, j, data$MinRelativeHumidity)
  if("MaxRelativeHumidity" %in% names(data)) .putvardatapixel( nc, ny, nt, "MaxRelativeHumidity", i, j, data$MaxRelativeHumidity)
  if("Radiation" %in% names(data)) .putvardatapixel( nc, ny, nt, "Radiation", i, j, data$Radiation)
  if("WindSpeed" %in% names(data)) .putvardatapixel( nc, ny, nt, "WindSpeed", i, j, data$WindSpeed)
  if("WindDirection" %in% names(data)) .putvardatapixel( nc, ny, nt, "WindDirection", i, j, data$WindDirection)
  if("PET" %in% names(data)) .putvardatapixel( nc, ny, nt, "PET", i, j, data$PET)
