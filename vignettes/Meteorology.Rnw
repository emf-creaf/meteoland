...
National meteorological agencies are increasingly adopting an open data philosophy and two of them are the Spanish meteorological agency (Agencia Española de Meteorología, AEMET) and the Catalan meteorologicl service (Servei Català de Meteorologia, SMC). In \texttt{meteoland} we provide functions that retrieve AEMET and SMC data using their OpenData API (API keys need to be obtained from AEMET or SMC):
\begin{itemize}
