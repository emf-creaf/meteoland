...
\begin{itemize}
\item{Function \texttt{spplot()} has been redefined from package \code{sp} to draw maps of specific weather variables corresponding to specific dates. The function can be used on objects of classs \texttt{SpatialGridMeteorology} and \texttt{SpatialPixelsMeteorology}.}
