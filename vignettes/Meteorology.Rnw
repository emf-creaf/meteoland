...
ml = interpolationgrid(exampleinterpolationdata, examplegridtopography,
                       as.Date(c("2000-03-23","2001-02-03", "2001-06-03")), verbose=FALSE)
...
dates = seq(as.Date("2002-01-01"), as.Date("2003-12-31"), by="day")
#Interpolation of two points for the whole time period (2000-2003)
mp = interpolationpoints(exampleinterpolationdata, spt, dates = dates, verbose=FALSE)
...
spplot(ml,1,"MeanTemperature")
@
\end{center}
And the minimum/average/maximum temperature daily series for a single point:
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 3.5>>=
meteoplot(mp,1,"MeanTemperature", xlab="", ylab="Temperature (ºC)", ylim=c(-10,40))
meteoplot(mp,1,"MinTemperature", add=TRUE, col="gray")
meteoplot(mp,1,"MaxTemperature", add=TRUE, col="gray")
@
\end{center}
...
spplot(ml,1,"MeanRelativeHumidity")
@
\end{center}
And the relative humidity daily series for a single point:
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 3.5>>=
meteoplot(mp,1,"MeanRelativeHumidity", xlab="", ylab="Relative humidity (%)", ylim=c(20,100))
meteoplot(mp,1,"MinRelativeHumidity", add=TRUE, col="gray", lty=1)
meteoplot(mp,1,"MaxRelativeHumidity", add=TRUE, col="gray", lty=1)
@
\end{center}
...
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 4>>=
spplot(ml,1,"Precipitation")
@
\end{center}
And the precipitation daily series for a single point:
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 3.5>>=
meteoplot(mp,1,"Precipitation", xlab="", ylab="Precipitation (mm)")
@
\end{center}
...
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 3.5>>=
meteoplot(mp,1,"WindSpeed", ylab=" Wind speed (m/s)", xlab="")
@
\end{center}
...
\begin{center}
<<echo=FALSE, fig=TRUE, width=6, height=3.5>>=
par(mar=c(5,5,1,5))
...
legend("topright", legend = paste0(c(3012,2012,1012,12), " AD"), col=c("brown","black","red","blue"), cex=0.8, lty=c(4,1,2,3), lwd=c(1,1.5,1,1), bty="n")
...
legend("topright", legend = c("40ºN","0º (equator)","40ºS"), cex=0.8, lty=1:3, bty="n")
...
for(i in 1:length(delta)) {
  inl1[i]=radiation_potentialRadiation(latrad1, 0, 0, delta[i])
  inl2[i]=radiation_potentialRadiation(latrad2, 0, 0, delta[i])
  inl3[i]=radiation_potentialRadiation(latrad3, 0, 0, delta[i])
}
...
for(i in 1:length(delta)) {
  in1[i]=radiation_potentialRadiation(latrad1, slorad, asprad1, delta[i])
  in2[i]=radiation_potentialRadiation(latrad1, slorad, asprad2, delta[i])
  in3[i]=radiation_potentialRadiation(latrad1, slorad, asprad3, delta[i])
  in4[i]=radiation_potentialRadiation(latrad1, slorad, asprad4, delta[i])
}
plot(dates, inl1, type="l", xlab="", ylab="Potential radiation (MJ·m-2·d-1)",
     main = "Slopes at 40ºNorth", ylim=c(0,45))
...
legend("bottom",c("flat", "N","E","S","W"), col=c("black",rep("gray",4)), lty=c(1,1:4), bty="n")
for(i in 1:length(delta)) {
  in1[i]=radiation_potentialRadiation(latrad2, slorad, asprad1, delta[i])
  in2[i]=radiation_potentialRadiation(latrad2, slorad, asprad2, delta[i])
  in3[i]=radiation_potentialRadiation(latrad2, slorad, asprad3, delta[i])
  in4[i]=radiation_potentialRadiation(latrad2, slorad, asprad4, delta[i])
}
...
legend("bottom",c("flat", "N","E","S","W"), col=c("black",rep("gray",4)), lty=c(1,1:4), bty="n")
for(i in 1:length(delta)) {
  in1[i]=radiation_potentialRadiation(latrad3, slorad, asprad1, delta[i])
  in2[i]=radiation_potentialRadiation(latrad3, slorad, asprad2, delta[i])
  in3[i]=radiation_potentialRadiation(latrad3, slorad, asprad3, delta[i])
  in4[i]=radiation_potentialRadiation(latrad3, slorad, asprad4, delta[i])
}
plot(dates, inl3, type="l", xlab="", ylab="Potential radiation (MJ·m-2·d-1)", 
     main = "Slopes at 40ºSouth", ylim=c(0,45))
lines(dates, in1, lty=1, col="gray")
lines(dates, in2, lty=2, col="gray")
lines(dates, in3, lty=3, col="gray")
lines(dates, in4, lty=4, col="gray")
...
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 4>>=
spplot(ml,1,"Radiation")
@
\end{center}
And the precipitation daily series for a single point:
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 4>>=
meteoplot(mp,1,"Radiation", xlab="", ylab="Solar radiation (MJ·m-2·d-1)")
@
\end{center}
...
R_nl[i] = radiation_outgoingLongwaveRadiation(latradp, elevationp, sloradp,aspradp, J[i], vpa, tmin[i], tmax[i], R_s[i])
R_net[i] = radiation_netRadiation(latradp, elevationp, sloradp,aspradp, J[i], vpa, tmin[i], tmax[i], R_s[i])
}
...
bc_dates = seq(as.Date("2022-01-01"), as.Date("2023-12-31"), by="day")
predicted = downscalingpoints(exampledownscalingdata, mp, spt@data, dates = bc_dates, verbose=FALSE)
@
...
meteoplot(predicted, 1, "Precipitation", ylab="Precipitation (mm)", ylim=c(0,80))
#Add uncorrected mean temperature data (cell #5)
lines(bc_dates,
      exampledownscalingdata@futuredata[[5]][as.character(bc_dates),"Precipitation"],
      col="red", lty=3)
legend("topleft", legend=c("corrected","uncorrected"), col=c("black","red"), lty=c(1,3), bty="n")
plot(exampledownscalingdata@futuredata[[5]][as.character(bc_dates),"Precipitation"], ylim=c(0,60), xlim=c(0,60),
...
#Add uncorrected mean temperature data (cell #5)
lines(bc_dates,
      exampledownscalingdata@futuredata[[5]][as.character(bc_dates),"MeanTemperature"],
      col="red")
legend("topright", legend=c("corrected","uncorrected"), col=c("black","red"), lty=c(1,1), bty="n")

#Scatter plot
plot(exampledownscalingdata@futuredata[[5]][as.character(bc_dates),"MeanTemperature"],
     predicted@data[[1]]$MeanTemperature, cex=0.1, asp=1,
     ylab="Corrected mean temperature", xlab="Uncorrected mean temperature")
abline(a=0,b=1,col="gray")
meteoplot(predicted, 1, "Radiation", ylab="Radiation (MJ·m-2)", ylim=c(0,40))
#Add uncorrected mean temperature data (cell #5)
lines(bc_dates,
      exampledownscalingdata@futuredata[[5]][as.character(bc_dates),"Radiation"],
      col="red")
legend("topright", legend=c("corrected","uncorrected"), col=c("black","red"), lty=c(1,1), bty="n")
#Scatter plot
plot(exampledownscalingdata@futuredata[[5]][as.character(bc_dates),"Radiation"],
...
\begin{center}
<<fig=TRUE, echo=FALSE, width = 4, height = 3.5>>=
meteoplot(mp,1,"PET", xlab="", ylab="PET (mm)")
@
\end{center}
