...
The station data downloaded looks like this:
```{r}
sg_st
```
...
The warning message indicates that default interpolation parameters are set and in a serious application we should calibrate the parameters before using the reference data for interpolation. The interpolator object is of class **stars**:
```{r}
interpolator_sen
```

If needed, we can display the location of the weather stations on a map with country limits using:
