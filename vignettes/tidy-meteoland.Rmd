...
```
  ...
  used. At any point, we can display the interpolation parameters using function `get_interpolation_params()`:
...
To load the interpolator in your session again, you can use the function `read_interpolator()`
function.
...
We can use function `interpolate_data()` in a raster type of data. All we need in this case is the topography information
...
Like with the point location example, interpolated data in raster format can also be aggregated temporaly.
