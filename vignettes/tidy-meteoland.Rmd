...
of `rgdal`, `rgeos` and `maptools` R packages, and the strong dependency `meteoland` has
with `sp` and `raster` R packages, a complete update of `meteoland` was necessary. This update
has removed any dependency on the retired packages as well as `sp` and `raster`, and now uses
`sf` and `stars` packages for working with simple features and raster data.

As fundamental changes were required (`meteoland` classes were based on `sp` and ones), a
...
raster_to_interpolate_example | Topological information for a 10x10 raster located in central Catalonia (Spain)
...
the only thing to consider is ensure that the topological variables has the mandatory names
(`elevation`, `aspect`, `slope`).

### Meteorological information

For interpolating the meteorological variables in our locations (see above), we need a
reference meteo data. This is a `sf` object with the reference locations, and the meteo
variables needed to perform the interpolation:
...
(`Precipitation`, `WindSpeed`...), when present, allow for interpolating also those variables.


**TODO**
preparing meteo data for meteoland

### Quick interpolation (if everythig is ok)

With the necessary data in the correct form we can perform the interpolation right away:
...
Interpolator objects can be reused for different interpolation in the same area covered
by the interpolator. To allow interpolator objects to be shared between sessions,
`meteoland` offers functions to write and read interpolator objects. The interpolator
is saved in NetCDF-CF format (https://cfconventions.org/cf-conventions/cf-conventions.html)
and can be opened with any GIS software that supports NetCDF-CF.
...
interpolation process. In fact, **is recommended** to calibrate the interpolator object
...
function.
    ...
    **Important!** Calibration process for one variable can take a long time to finish,
    ...
    interpolator. In this example we reduce the N and alpha test values for the process
    to be faster, but is recommended to explore a wider range of N and alpha values.

```{r interpolator_calibration, cache=TRUE}
...
```{r preparing_interpolator, cache=TRUE}
interpolator <- with_meteo(meteoland_meteo_example) |>
  create_meteo_interpolator() |>
...
interpolated_points <- points_to_interpolate_example |>
  interpolate_data(interpolator) |>
  summarise_interpolated_data(
    fun = "mean",
    frequency = "week"
